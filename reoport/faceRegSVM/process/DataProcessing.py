...
def image_data(datafolder, path_train, path_test):
    X = []
    Y = []
    print("Is dividing the trainControl test ...")
    ...
    print("Number of photo trainControl:", len(X_train), "and Number of photo test:", len(X_test))
...
def ImageFileCount(path_train, path_test):
    for path in os.listdir(path_train):
        ...
        for file in os.listdir(curr_path):
            ...
            CountImage += 1
        print("folder", curr_path, "la:", CountImage, "data to trainControl")
    print(CountImage)
    list_name = []
    ...
    for cout_name in set_name:
        ...
        for name in list_name:
            if name == cout_name:
                result += 1
        print("folder", cout_name, "using", result, "data to test")
...
def CheckMaxMin(path_train):
    ...
    for folder in os.listdir(path_train):
        ...
        if len(os.listdir(person)) > filemax:
            ...
            foldermax = folder
    print('The least picture folder is the folder', foldermin, 'with data number is:', filemin)
    print("The folder with the most pictures is the folder", foldermax, 'with data number is:', filemax)
